...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='本地幕墙平整度检测')
...
def save_mask(binary_mask: np.ndarray, output_dir: Path, input_path: Path) -> str:
    mask_path = output_dir / f'{input_path.stem}-mask.png'
...
def save_overlay(image: Image.Image, glass_reports: list[dict[str, Any]], output_dir: Path, input_path: Path) -> str:
    ...
    overlay_path = output_dir / f'{input_path.stem}-flatness-overlay.png'
...
def print_report(report: dict[str, Any]) -> None:
    ...
    print(f"玻璃区域数量: {report['glass_count']}")
    print(f"平整区域数量: {report['flat_glass_count']}")
    print(f"不平整区域数量: {report['uneven_glass_count']}")
    print(f"分割掩码: {report['outputs']['mask']}")
    print(f"标注图: {report['outputs']['overlay']}")
