...
func findProjectDetectionCrackTaskByUuidTx(ctx context.Context, tx *sql.Tx, taskUuid string) (*model.ProjectDetectionSubTaskRecord, error) {
...
func createProjectDetectionCrackTaskTx(ctx context.Context, tx *sql.Tx, task *model.ProjectDetectionTaskRecord) (*model.ProjectDetectionSubTaskRecord, error) {
...
func updateProjectDetectionCrackTaskResultTx(ctx context.Context, tx *sql.Tx, taskUuid string, status bizpb.ProjectDetectionSubTaskStatus_Value, resultJSON string) error {
	...
	result, err := tx.ExecContext(ctx, `
		...
		SET status = ?,
			...
			finished_at = NOW(3)
		WHERE uuid = ?
	`, statusText, report.HasCrack, report.CrackCount, report.CrackPixels, report.CrackRatio, utils.JsonOrEmptyArray(report.Regions), report.RuntimeSeconds, report.RuntimeSeconds, taskUuid)
