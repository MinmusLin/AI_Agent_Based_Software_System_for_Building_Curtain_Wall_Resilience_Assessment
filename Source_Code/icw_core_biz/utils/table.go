...
func FormatTable(columns []TableColumn) string {
	...
	builder.WriteString("\n")
	builder.WriteString(formatTableRow(tableHeaders(columns), widths))
	builder.WriteString("\n")
	builder.WriteString(border)
	builder.WriteString("\n")
	for rowIndex := 0; rowIndex < rowCount; rowIndex++ {
		builder.WriteString(formatTableRow(tableRow(columns, rowIndex), widths))
		builder.WriteString("\n")
	}
	builder.WriteString(border)
...
// tableHeaders 将表格列数据格式化为终端表格标题行
...
// tableRow 将表格列数据格式化为终端表格数据行
func tableRow(columns []TableColumn, rowIndex int) []string {
