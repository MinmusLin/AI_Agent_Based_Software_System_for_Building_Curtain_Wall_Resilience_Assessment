...
func RegisterRPCServices(serviceDeps *common.Deps) {
	// 注册用户业务 Service
	...
	// 注册项目核心 Service
	projectCoreService := core.NewService(serviceDeps)
	...
	// 注册基础信息 Service
	projectProfileService := profile.NewService(serviceDeps)
	...
	// 注册图像资产 Service
	projectAssetsService := assets.NewService(serviceDeps)
	...
	// 注册智能检测 Service
	projectDetectionService := detection.NewService(serviceDeps)
	...
	// 注册人工复核 Service
	projectReviewService := review.NewService(serviceDeps)
	...
	// 注册评估报告 Service
	projectReportService := report.NewService(serviceDeps)
