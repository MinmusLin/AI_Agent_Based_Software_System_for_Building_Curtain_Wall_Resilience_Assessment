...
func (s *Service) uploadProjectImage(req *bizpb.UploadProjectImageRequest, resp *bizpb.UploadProjectImageResponse) error {
	...
	for _, image := range req.Images {
		...
		if fileName == "" {
			return rpc_error.BadRequestDefault("project image file name is required")
		}
		if utf8.RuneCountInString(fileName) > consts.ProjectImageFileNameMaxLength {
			return rpc_error.BadRequest(rpc_error.DetailProjectImageFileNameTooLong, "project image file name is too long")
		}
		if image.SizeBytes == 0 || image.Width == 0 || image.Height == 0 {
			return rpc_error.BadRequest(rpc_error.DetailProjectImageFormatInvalid, "project image format is invalid")
		}
