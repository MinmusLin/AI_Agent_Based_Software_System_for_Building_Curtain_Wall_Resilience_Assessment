...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='本地幕墙污渍检测')
...
def build_outputs(input_path: Path, image: np.ndarray, results: Any, output_dir: Path) -> dict[str, Any]:
    ...
    if not detections:
        return {
            ...
            'message': '模型检测到了掩码，但未能成功处理任何幕墙块。',
        }

    annotated_path = annotated_dir / f'{input_path.stem}-annotated.jpg'
...
def print_report(report: dict[str, Any]) -> None:
    print('\n==== 本地污渍检测报告 ====')
    print(f"输入图片: {report.get('input')}")
    print(f"状态: {report['status']}")
    if report['status'] != 'success':
        print(f"错误码: {report.get('error_code')}")
        print(f"说明: {report.get('message')}")
        return
    image_size = report['image_size']
    print(f"图片尺寸: {image_size['width']} x {image_size['height']} px")
    print(f"幕墙块数量: {report['total_blocks']}")
    print(f"平均污渍占比: {report['average_stain_percentage']}%")
    print(f"最大污渍占比: {report['max_stain_percentage']}%")
    print(f"标注图: {report['annotated_image']}")
