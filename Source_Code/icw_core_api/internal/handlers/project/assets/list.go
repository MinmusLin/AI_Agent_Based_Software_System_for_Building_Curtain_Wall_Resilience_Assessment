...
func (h *Handler) GetProjectAssets(c *gin.Context) {
	...
	rpcReq := &bizpb.GetProjectAssetsRequest{
		UserId:    user.Id,
		ProjectId: projectId,
	}
	rpcResp := &bizpb.GetProjectAssetsResponse{}
	if err := project_assets.GetProjectAssets(c.Request.Context(), h.CoreBizClient(), rpcReq, rpcResp); err != nil {
		response.WriteError(c, err)
		return
	}

	response.OK(c, apiUtils.NewGetProjectAssetsResponse(rpcResp))
}
