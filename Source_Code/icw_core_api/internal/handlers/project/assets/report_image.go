...
func (h *Handler) ReportProjectImage(c *gin.Context) {
	var req apipb.ReportProjectImageRequest
	...
	rpcReq := &bizpb.ReportProjectImageRequest{
		UserId:    user.Id,
		ProjectId: projectId,
		ImageUuid: req.ImageUuid,
		Status:    req.Status,
	}
	rpcResp := &bizpb.ReportProjectImageResponse{}
	if err := project_assets.ReportProjectImage(c.Request.Context(), h.CoreBizClient(), rpcReq, rpcResp); err != nil {
		response.WriteError(c, err)
		return
	}

	response.OK(c, apiUtils.NewReportProjectImageResponse(rpcResp))
}
