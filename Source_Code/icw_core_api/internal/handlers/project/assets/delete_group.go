...
func (h *Handler) DeleteProjectGroup(c *gin.Context) {
	var req apipb.DeleteProjectGroupRequest
	...
	rpcReq := &bizpb.DeleteProjectGroupRequest{
		UserId:    user.Id,
		ProjectId: projectId,
		GroupId:   groupId,
	}
	rpcResp := &bizpb.DeleteProjectGroupResponse{}
	if err := project_assets.DeleteProjectGroup(c.Request.Context(), h.CoreBizClient(), rpcReq, rpcResp); err != nil {
		response.WriteError(c, err)
		return
	}

	response.OK(c, apiUtils.NewDeleteProjectGroupResponse(rpcResp))
}
