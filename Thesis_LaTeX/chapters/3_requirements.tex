...
为了将建筑幕墙韧性评估从人工经验流程转化为软件系统，首先需要明确业务对象与用户流程。业务对象包括项目、图像组、图像、检测主任务、检测子任务、图像总结、人工复核和项目报告等。项目保存建筑名称、位置、建设年份、既有问题和评估目标等背景信息；图像组对应建筑立面、楼层或采集区域；图像保存原图、尺寸、文件大小和上传状态；检测主任务以单张图像为单位，关联分类路由、五类检测子任务和图像总结；人工复核提供专业的判断依据；项目报告则是最终面向运维决策的输出。用户流程可以抽象为项目基础信息、图像资产构建、Agent 智能检测、人工复核确认和评估报告生成五个阶段，各阶段之间存在严格依赖关系。基于上述对象模型和流程模型，本章进一步分析角色责任、典型用例、功能需求、非功能需求和相关约束条件。
...
需要说明的是，UML 用例图强调参与者与系统功能边界，但不直接表达严格执行顺序。本文系统在真实使用中仍然遵循项目基础信息、图像资产构建、Agent 智能检测、人工复核确认和评估报告生成的阶段化流程，具体阶段约束将在后续需求分析中展开。
...
该数据流说明系统中至少同时存在三类核心数据：文件数据、结构化数据和语义数据。文件数据包括原图、缩略图、检测产物和报告源文件，存储在 MinIO；结构化数据包括项目、图像、任务状态、指标字段、区域 JSON 和复核意见，存储在 MySQL；语义数据包括图像级总结和项目评估报告，由 Agent 生成后经 BIZ 校验并写入数据库。三类数据分别服务于不同目标：文件用于工程复核，结构化数据用于过程追踪和页面展示，语义数据用于报告生成和运维决策。需求分析必须同时覆盖三类数据及其流转关系，否则系统无法形成从视觉证据到工程结论的可追溯链路。
