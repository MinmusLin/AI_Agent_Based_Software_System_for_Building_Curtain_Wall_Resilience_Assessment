...
除功能闭环外，本文系统还需要满足可扩展性、一致性、可观测性、可部署性和用户体验等非功能需求。这些需求与\cref{sec:foundation-agent-engineering-controllability-problem}讨论的 Agent 工程化问题密切相关：Agent 输出具有不确定性，检测和报告任务耗时较长，图像产物规模较大，跨服务调用链路复杂。如果只关注页面功能而忽略非功能需求，系统即使能够完成一次演示，也难以稳定支撑真实场景试用。
...
根据上述需求，本文系统实现并非简单 CRUD，而是在多个维度存在复杂性。

第一，业务链路长。用户从上传图像到生成报告，需要经历资产管理、分类路由、并发检测、图像总结、人工复核和项目报告多个阶段。

第二，服务协作复杂。系统包含前端、API、BIZ、三个 Activity 服务、数据库、对象存储、缓存和消息队列等。

第三，状态同步复杂。检测状态既要写入数据库，又要通过 RocketMQ 和 WebSocket 推送给前端，还要在页面刷新后可恢复。

第四，结果结构复杂。不同检测任务输出字段不同，区域数量不定，产物图像数量不定，报告生成还需要将这些结果重新组织。
