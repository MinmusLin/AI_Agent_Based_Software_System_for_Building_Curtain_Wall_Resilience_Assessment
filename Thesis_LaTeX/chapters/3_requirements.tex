...
数据流分析用于说明系统中的数据如何从用户输入转化为可复核的检测结果和可阅读的评估报告。与普通信息管理系统不同，本文系统同时包含前端交互数据、业务状态数据、图像文件数据、模型推理结果、Agent 语义输出和实时事件数据。为了体现这些数据之间的关系，\cref{fig:requirements-system-data-flow} 从 FE 前端、API 接入层、BIZ 业务层、Activity 基础能力服务和基础设施五个层次刻画系统主要数据流。
...
\begin{figure}[!htb]
  ...
  \resizebox{\linewidth}{!}{%
    \begin{tikzpicture}[
      ...
      \node[storage, fill=yellow!14, minimum width=2.35cm] (websocket) at (1.92, 1.40) {WebSocket\\实时状态通道};
...
数据流中还存在一条独立的实时事件链路。BIZ 在状态变化后向 RocketMQ 发布项目事件，Core API 消费事件后通过 WebSocket 广播给前端，使用户能够实时看到图像上传、检测进度和报告状态变化。页面刷新或 WebSocket 断线后，前端仍然可以重新调用列表和详情接口，从 MySQL 中恢复完整状态。因此，RocketMQ 和 WebSocket 主要承担增量通知职责，数据库中的结构化状态仍然是系统最终可信来源。

该数据流说明系统中至少同时存在三类核心数据：文件数据、结构化数据和语义数据。文件数据包括原图、缩略图、检测产物和报告源文件，存储在 MinIO；结构化数据包括项目、图像、任务状态、指标字段、区域 JSON 和复核意见，存储在 MySQL；语义数据包括单图总结和项目评估报告，由 Agent 生成后经 BIZ 校验并写入数据库。三类数据分别服务于不同目标：文件用于工程复核，结构化数据用于过程追踪和页面展示，语义数据用于报告生成和运维决策。需求分析必须同时覆盖三类数据及其流转关系，否则系统无法形成从视觉证据到工程结论的可追溯链路。
...
上述需求分析为\cref{chap:design}技术方案设计和\cref{chap:implementation}系统架构与工程实现提供了明确依据。\cref{chap:design}将重点回答如何设计面向建筑幕墙多材质图像的 Agent 自主任务调度方法和 LLM 分层信息整合策略，\cref{chap:implementation}则进一步说明这些需求如何落地为服务架构、协议、数据库状态机、对象存储和前端可视化实现。
