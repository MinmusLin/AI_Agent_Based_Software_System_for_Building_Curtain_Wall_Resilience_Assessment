...
为了避免后续表述产生歧义，本章首先对使用的关键术语进行说明。这里的术语并非孤立概念，而是对应系统中真实存在的设计对象、服务节点和数据结构。
...
\begin{table}[!htb]
  ...
  \begin{tabular}{@{}p{0.22\linewidth}p{0.68\linewidth}@{}} \toprule
    ...
    LLM 分层信息整合策略 & 将原子检测结果、单图总结、人工复核和项目上下文分层组织，逐级生成图像级总结和项目级评估报告的策略。 \\
    ...
    检测主任务 & 以单张项目图像为单位创建的检测业务记录，串联分类、子检测、单图总结和最终状态。 \\
...
\begin{figure}[!htb]
  ...
  \resizebox{\linewidth}{!}{%
    \begin{tikzpicture}[
      ...
      \node[llmcell, minimum width=1.85cm] at (-1.142, 3.37) {图像语义层\\单图总结};
...
图中箭头反映了总体方法的主线：业务需求首先牵引 Agent 自主任务调度层进行任务选择，项目资产进入数据证据与上下文层形成结构化上下文；基础支撑和工程治理层为上层提供状态、协议、对象授权、事务落库和失败恢复能力；原子检测工具层生成可追溯的检测事实；LLM 分层信息整合层在结构化输入基础上完成单图总结与项目报告整合；目标输出层最终面向用户呈现单图检测总结、项目评估报告、区域风险等级和维护改进建议。该框架的价值在于把不确定的语义判断限制在受控 Agent 节点中，把确定性的状态推进、数据管理和质量校验交还给工程系统。
...
Agent 自主任务调度层解决“哪些图像需要执行哪些检测”的问题。该层不直接输出缺陷结论，而是根据幕墙场景识别和材质语义识别结果生成有限任务代码，并通过枚举归一、去重排序、子任务创建策略和失败回调重试入口，把自然语言语义判断转换为可执行的工程动作。LLM 分层信息整合层解决“检测结果如何转化为项目级认知”的问题。该层将原子事实层的指标与产物、图像语义层的单图总结、项目报告层的多源融合和报告结构层的固定字段分开处理，并通过结构化数据输入、输出压缩 JSON 校验和一致性约束降低输出漂移与事实编造风险。
...
在本系统中，Agent 不是一个开放式聊天入口，也不是可以自主操作数据库的通用代理。本系统将 Agent 限定为业务流程中的三个受控节点：分类 Agent、图像总结 Agent 和项目报告 Agent。三者对应从“任务选择”到“局部解释”再到“整体评估”的递进关系。
...
\begin{table}[!htb]
  ...
  \begin{tabular}{@{}p{0.14\linewidth}p{0.26\linewidth}p{0.28\linewidth}p{0.22\linewidth}@{}} \toprule
    ...
    分类 Agent & 待检测幕墙图像与路由提示词 & 检测任务代码列表 & 判断该图像应执行哪些原子检测能力 \\
    图像总结 Agent & 单张图像的结构化检测结果 & 固定字段的单图检测总结 JSON & 将底层指标压缩为可读的图像级语义 \\
    项目报告 Agent & 项目基础信息、图像组、单图总结、检测结果和复核意见 & 固定字段的项目评估报告 JSON & 形成建筑幕墙韧性评估结论和维护建议 \\ \bottomrule
  \end{tabular}
\end{table}
...
本文在方案设计阶段形成了六类可选技术路线。它们都具有一定合理性，但适用范围和风险不同。为突出选择过程，本文从“如何选择检测任务”和“如何生成项目报告”两个关键问题出发进行比较。
  ...
  \resizebox{0.94\linewidth}{!}{%
    \begin{tikzpicture}[
      ...
      \node[contract, minimum height=1.08cm] (governance) at (0, -4.88) {
        \textbf{任务代码治理}\\[2pt]
        非幕墙过滤 \quad 非法任务代码拦截 \quad 去重 \quad 固定顺序排序 \quad 空集合允许
      };
...
\begin{table}[!htb]
  \caption{幕墙材质语义与任务代码映射关系}\label{tab:design-material-task-code-mapping}
  \centering
...
LLM 分层信息整合策略是本文从单图检测走向项目级韧性评估的关键设计。旧式做法通常把每个模型的检测结果直接展示给用户，用户再人工汇总；另一种极端做法是把所有检测结果一次性输入大模型并要求其生成报告。前者无法解决报告滞后问题，后者容易受到上下文长度、低层指标噪声和输出不稳定影响。本文采用分层整合方案，将信息处理划分为原子事实层、图像语义层和项目报告层。

\subsection{分层整合的必要性}
\label{subsec:design-llm-hierarchy-necessity}

项目级幕墙评估与普通文本摘要不同。输入数据既包括项目基础信息，又包括图像组、图像元数据、五类检测指标、区域列表、图像产物、单图总结和人工复核意见。如果将所有底层 JSON 直接输入项目报告 Agent，一方面会造成上下文过长，另一方面低层细节会干扰模型对项目级风险的组织。相反，如果只把单图总结输入项目报告 Agent，又会损失必要的结构化指标和复核依据。

本文因此采用分层整合。原子检测能力首先产生可追溯事实，图像总结 Agent 将每张图像的事实压缩为语义概要，项目报告 Agent 再同时读取项目结构、检测事实、图像总结和人工复核意见。这样既避免项目报告直接处理过多像素级或区域级细节，又保留必要的证据字段。

\begin{table}[!htb]
  \caption{LLM 分层信息整合策略}\label{tab:design-llm-hierarchical-integration}
  \centering
  \begin{tabular}{@{}p{0.17\linewidth}p{0.34\linewidth}p{0.39\linewidth}@{}} \toprule
...
在单图总结设计中，系统不会将未执行的检测任务描述为“未发现缺陷”。例如，分类 Agent 没有选择金属锈蚀检测时，单图总结不应写“未发现锈蚀”，因为系统并没有对该图像执行锈蚀检测。该规则能够避免报告中出现证据范围之外的结论。对于已执行但未发现缺陷的任务，则可以在总体概述中说明已执行任务未提示明显异常，但对应缺陷字段保持为空。
...
项目报告生成设计的关键不在于让 Agent 写长文本，而在于如何把多源数据组织成可供 Agent 稳定阅读的上下文。Core BIZ 会先将项目基础信息、图像组、图像检测结果、单图总结和复核意见组装为结构化 JSON，并以 \texttt{source.txt} 文件形式存入对象存储。项目报告 Agent 通过预签名 URL 下载该文件并作为附件读取。
...
\begin{table}[!htb]
  ...
  \centering
  \begin{tabular}{@{}p{0.24\linewidth}p{0.58\linewidth}@{}} \toprule
...
\cref{tab:design-project-report-source-data} 中的设计体现了“去无用信息、保留工程语义”的原则。系统不会把数据库中的所有字段都传给 Agent，例如内部 ID、任务主键、产物 SHA256、运行耗时和部分图像坐标对项目报告意义有限，可以不进入报告上下文；但缺陷指标、区域数量、图像组、人工复核和单图总结都与工程判断相关，应完整保留。这样既减少上下文噪声，又保证报告内容翔实。

\subsection{项目级报告输出契约}
\label{subsec:design-project-report-output-contract}

项目级报告输出必须是结构化 JSON，而不是普通 Markdown 文本。原因在于前端需要按照字段展示项目背景、韧性评估目标、数据集概述、总体评估结论、总体风险等级、区域评估结论、总体韧性评估、维护和改进建议以及限制说明。结构化输出可以避免前端解析自然语言段落，也便于后续扩展报告打印、导出和字段化展示。
...
\subsection{分层整合与工程认知路径}
\label{subsec:design-engineering-cognition-path}

该策略体现了工程报告生成的认知路径。实际工程师撰写项目报告时，也不会直接从每个像素坐标开始写，而是先理解每张图像的主要现象，再比较不同区域之间的缺陷分布，最后给出整体风险和维护优先级。LLM 分层信息整合策略把这一过程固化为软件流程，使 Agent 的语言能力服务于工程语义组织，而不是替代检测事实。
...
\begin{table}[!htb]
  \caption{检测主任务状态含义}\label{tab:detection-state-meaning}
  \centering
  \begin{tabular}{@{}p{0.18\linewidth}p{0.72\linewidth}@{}} \toprule
    ...
    \texttt{summarizing} & 子任务结果已满足总结条件，图像总结 Agent 正在生成单图总结。 \\
...
在人机协同方面，本文在检测结束后引入人工复核阶段。用户可以查看检测产物、区域指标和 Agent 总结，对结果选择“准确”或“不准确”，也可以填写补充评论。复核信息不会删除或覆盖原始检测事实，而是作为项目报告输入的一部分。这样既保留自动检测证据，也保留人的判断痕迹。
...
本章提出了面向建筑幕墙韧性评估的技术方案设计。方案以 Agent 自主任务调度方法和 LLM 分层信息整合策略为核心，以原子检测能力工具化和业务状态机为支撑。分类 Agent 将图像语义映射为有限任务代码，Reasoning 服务将既有检测模型封装为可调度工具，图像总结 Agent 和项目报告 Agent 将底层检测结果逐级转化为工程语义，人工复核机制则保证最终结论能够吸收人的判断。

与端到端大模型方案相比，本文方案链路更长，但每个环节都有明确边界、证据来源和失败恢复路径。与固定检测流程相比，本文方案能够根据图像材质自动选择检测能力，减少无意义模型调用。与普通工程系统相比，本文方案引入 Agent 处理语义判断和信息整合，使系统能够从单图单缺陷检测扩展到项目级韧性评估。第五章将在本章方法设计基础上，进一步说明多服务架构、协议、数据库、对象存储、Worker、回调、消息队列、WebSocket、CronJob 和部署配置如何共同支撑这一方案落地。
