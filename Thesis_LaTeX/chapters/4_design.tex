...
本文因此采用分层整合。原子检测能力首先产生可追溯事实，图像总结 Agent 将每张图像的事实压缩为语义概要，项目报告 Agent 再同时读取项目结构、检测事实、图像总结和人工复核意见等。这样既避免项目报告直接处理过多像素级或区域级细节，又保留必要的证据字段。
...
\cref{tab:design-project-report-source-data} 中的设计体现了“去除无效噪声、保留工程语义”的原则。系统不会将数据库中的所有字段都传给 Agent，而是先剔除内部 ID、任务主键、产物 SHA256、运行耗时和部分图像坐标等对项目报告意义有限的字段，避免这些底层实现细节干扰报告生成。与工程判断直接相关的信息则需要保留，例如缺陷指标、区域数量、图像组结构、人工复核意见和单图总结。通过这种取舍，报告源数据既能减少上下文噪声，又能保留形成建筑级韧性评估结论所需的关键依据。
  ...
  \resizebox{0.94\linewidth}{!}{%
    \begin{tikzpicture}[
      ...
      \node[summary] (summarizing) at (0, -4.88) {
        \textbf{图像级总结生成}\\[2pt]
        \texttt{summarizing} \quad 基于已落库检测结果生成单图总结
      };
...
本章提出了面向建筑幕墙韧性评估软件系统的技术方案设计。方案以 Agent 自主任务调度方法和 LLM 分层信息整合策略为核心，以原子检测能力工具化和业务状态机为支撑。分类 Agent 将图像语义映射为有限任务代码，Reasoning 服务将既有检测模型封装为可调度工具，图像总结 Agent 和项目报告 Agent 将底层检测结果逐级转化为工程语义，人工复核机制则保证最终结论能够吸收专业判断。

与端到端大模型方案相比，本系统技术方案链路更长，但每个环节都有明确边界、证据来源和失败恢复路径。与固定检测流程相比，本文方案能够根据图像材质自动选择检测能力，减少无意义模型调用。与普通工程系统相比，本文方案引入 Agent 处理语义判断和信息整合，使系统能够从单图单缺陷检测扩展到项目级韧性评估。\cref{chap:implementation}将在本章技术方案的设计基础上，进一步说明多服务架构、协议、数据库、缓存、对象存储、Worker、回调、消息队列、WebSocket、CronJob 和部署配置如何共同支撑这一技术方案落地。
