...
\section{数据实体与状态机实现}
\label{sec:implementation-database-and-state-machine-implementation}

数据库设计围绕“项目级资产”和“图像级任务”展开。项目表保存项目名称、建筑名称、位置、建成年份、建筑描述、已知问题、评估目标、项目状态和项目进度；图像组表保存项目内区域或用户自定义分组；图像表保存图像 UUID、文件名、尺寸、大小、上传状态和元数据；检测主任务表保存每张图像的检测整体状态、五类子任务是否执行、子任务 ID、图像总结任务 ID 和人工复核字段；五张子任务表分别保存不同原子检测能力的专属指标；图像总结任务表保存单图总结；项目报告表保存项目级报告 JSON 和状态。

\begin{table}[!htb]
  \caption{核心数据实体与作用}\label{tab:implementation-core-entities}
  \centering
  \begin{tabular}{@{}p{0.28\linewidth}p{0.62\linewidth}@{}} \toprule
    \textbf{数据实体} & \textbf{作用} \\ \midrule
    \texttt{projects} & 保存项目基础信息、项目状态和阶段进度，是所有业务数据的归属边界。 \\
    \texttt{project\_groups}、\texttt{project\_group\_images} & 保存图像组、图像元数据、图像状态和图像资产组织关系。 \\
...
检测主任务状态机是数据层设计的核心。主任务状态包括 \texttt{pending}、\texttt{classifying}、\texttt{detecting}、\texttt{summarizing}、\texttt{succeeded} 和 \texttt{failed}。子任务状态包括 \texttt{pending}、\texttt{succeeded} 和 \texttt{failed}。报告状态包括 \texttt{pending}、\texttt{succeeded} 和 \texttt{failed}。项目阶段进度从项目初始化、基础信息完成、图像资产完成、检测完成、复核完成推进到项目完成。只有报告成功生成后，项目才允许被视为完成。
...
\section{对象存储与产物管理}
\label{sec:implementation-object-storage-and-artifact-management}

系统使用 MinIO 保存项目缩略图、图像原图、图像缩略图、检测产物和项目报告源数据。数据库不保存二进制文件，只保存元数据、状态和必要的产物校验信息。前端上传图像时，BIZ 生成预签名 PUT URL，前端将原图和缩略图直接上传到对象存储，再调用图像上报接口确认状态。检测产物上传时，Reasoning 服务向 BIZ 获取受限前缀上传授权，在指定路径下上传动态数量产物。

对象存储路径按项目、图像和任务组织。图像资产使用 \texttt{projects/\{project\}/assets/\{image\}/} 前缀，检测产物使用 \texttt{projects/\{project\}/detections/\{image\}/\{task\}/} 前缀，项目报告源数据使用 \texttt{projects/\{project\}/reports/source.txt}。这样的路径结构便于按项目、图像或任务维度清理和追踪。
...
Redis 用于缓存预签名下载 URL 和临时控制状态。由于图像和产物可能被前端反复查看，缓存可以减少重复生成 URL 的开销。删除图像或清理脏对象时，系统会清理对应 Redis 缓存，避免前端继续使用过期对象地址。

\section{安全性设计与实现}
\label{sec:implementation-security-design-and-implementation}

安全性设计覆盖用户身份、项目权限、对象存储访问、WebSocket 连接和服务边界。用户登录后获得访问令牌和刷新令牌，API 鉴权中间件校验访问令牌并向 BIZ 查询用户信息。刷新令牌由 BIZ 管理，配合过期时间和刷新状态控制，避免多个请求并发刷新造成令牌混乱。
...
Agent 输出也需要安全性和可靠性约束。分类输出必须被解析为受控任务代码；总结输出必须被压缩为合法 JSON 对象；项目报告输出还需要校验图像组数量和图像数量是否与源数据一致。Reasoning 服务启动时校验模型文件，避免部署中只存在 Git LFS 指针而非真实模型权重。通过这些约束，系统将外部 Agent 和模型的非确定性限制在可处理范围内。

\section{稳定性设计与实现}
\label{sec:implementation-stability-design-and-implementation}

系统稳定性主要体现在日志、异常处理、容量控制、超时控制、重试恢复和清理机制上。日志方面，各服务按 HTTP、RPC、MQ、WS、CRON、CLS、RSN、SMR 等范围输出结构化日志，并保留请求 ID、任务 UUID、图像 UUID、状态、耗时和错误信息。Docker Compose 中为五个后端服务配置日志采集 sidecar，将容器日志按 PSM 写入 \texttt{/root/logs/\{psm\}/log}，并按文件大小和最大文件数量进行轮转。
