...
本章涉及的对象更多偏向工程实现，包括服务模块、通信方式、存储对象和运维机制等。为便于阅读，现在\cref{tab:implementation-key-terms} 中对本章关键术语进行说明。
...
系统采用前后端分离与微服务架构。前端 Core Web 负责项目工作台、项目阶段页面、图像资产管理、检测进度展示、检测结果查看、人工复核和报告展示；后端按照职责拆分为 Core API、Core BIZ、Activity Classification、Activity Reasoning、Activity Summary 和 ICW Common；基础设施包含 MySQL、Redis、MinIO 和 RocketMQ 等；部署层由 Docker Compose 统一组织容器、网络、环境变量、日志采集和依赖启动顺序。
...
\begin{figure}[!htb]
  ...
  \resizebox{\textwidth}{!}{%
    \begin{tikzpicture}[
      ...
      userbox/.style={region, draw=blue!60!black, fill=blue!5},
      corebox/.style={region, draw=green!55!black, fill=green!6},
      ...
      usercell/.style={cell, draw=blue!60!black, fill=blue!9},
      corecell/.style={cell, draw=green!55!black, fill=green!10},
...
从工程复杂度看，用户在页面上点击一次“启动 Agent 智能检测”，后端并不是执行一个简单接口。请求先由 Core Web 发起，经 Core API 完成鉴权后进入 Core BIZ；随后 Core BIZ 为图像创建检测主任务，并交给 Worker 异步推进。Worker 会先调用分类 Agent，等分类结果回调后再创建多个检测子任务；Reasoning 服务负责执行 Python 模型并上传检测产物，Summary 服务再基于检测结果生成图像级总结。与此同时，BIZ 还需要发布 RocketMQ 事件，API 消费事件后通过 WebSocket 把状态推送给前端。也就是说，一次用户操作会被拆成多个后台步骤，系统需要在并发执行、回调延迟或某个环节失败时，仍然保持数据库状态和前端状态一致。
...
RocketMQ 主要承担业务状态变化后的项目事件转发。Core BIZ 在图像上传、检测主任务、检测子任务、图像级总结和项目报告等状态发生变化后，会把项目 ID、事件类型和事件载荷写入统一 Topic。Core API 订阅该 Topic 后，再根据事件类型转换成前端需要的 WebSocket Scope。这样划分后，BIZ 不需要关心当前有哪些客户端在线，只要在事务完成后发布已经发生的事实事件；API 也不参与业务事务，只负责消费事件并向前端广播。长任务回调不会直接占用 HTTP 请求，页面中的图像资产、检测进度和报告状态也可以通过增量事件更新。

WebSocket 连接采用“连接票据 + Scope”的设计。前端建立连接前，先通过鉴权 HTTP 请求申请连接票据，然后携带项目 ID、Scope 和 Ticket 发起 WebSocket 连接。这里的 Scope 用来区分图像资产、检测任务和报告状态，避免某个页面收到与当前视图无关的全部事件。API 侧 Hub 按项目和 Scope 维护连接集合；当 RocketMQ 消费到项目事件后，只把消息广播给对应项目、对应 Scope 下的客户端即可。
...
Core API 位于前端和业务服务之间，主要负责把页面请求接入后端系统。实现上，该服务基于 Gin 注册 HTTP 路由，认证、用户头像、项目核心流程、项目基础信息、图像资产、智能检测、人工复核、评估报告和 WebSocket 连接票据等接口都由这一层对外暴露。请求进入后，API 先通过鉴权中间件解析令牌中的用户身份，再通过项目访问权限中间件检查用户是否可以访问当前项目。通过校验后，请求才会被转换为 Core BIZ 的 gRPC 调用。

实时状态更新也由 Core API 负责承接。Core BIZ 不直接保存前端连接信息，它只在图像状态、检测任务状态和报告状态变化后，把项目事件写入 RocketMQ。Core API 订阅这些项目事件，再根据事件类型选择对应的 WebSocket Scope，并广播给相应项目下的前端连接。这样划分后，BIZ 可以专注业务状态和事务处理，WebSocket 连接、事件消费和消息推送则集中在 API 层完成。
...
\begin{figure}[!htb]
  ...
  \resizebox{\linewidth}{!}{%
    \begin{tikzpicture}[
      ...
      \node[service] (auth) at (4.50, 0) {登陆鉴权与\\项目权限};
      ...
      \draw[msg] (0,-0.90) -- node[label, above] {HTTP 请求 / 携带 Token 与 ProjectId} (2.17,-0.90);
      \draw[msg] (2.33,-1.55) -- (3.12,-1.55) -- node[label, right, pos=0.5] {路由匹配 / 绑定请求参数 / 生成 RequestId} (3.12,-2.15) -- (2.33,-2.15);
      \draw[msg] (2.33,-2.85) -- node[label, above] {解析用户身份 / 校验项目权限} (4.42,-2.85);
      \draw[ret] (4.42,-3.45) -- node[label, below] {UserId / Project 权限结果} (2.33,-3.45);
      \draw[msg] (2.33,-4.15) -- node[label, above] {封装 gRPC Payload / Metadata 透传 RequestId} (6.67,-4.15);
      ...
      \draw[msg] (9.08,-8.05) -- node[label, above] {API 消费事件并解析 EventType / Scope} (11.17,-8.05);
      \draw[msg] (11.17,-8.75) -- node[label, above] {按 ProjectId + Scope 广播} (0,-8.75);
    \end{tikzpicture}
  }
  \caption{Core API 服务核心时序图}\label{fig:implementation-api-service-sequence}
\end{figure}

如\cref{fig:implementation-api-service-sequence} 所示，Core API 的重点在于实现前端页面中同步查询和异步事件的统一。首次进入页面或手动刷新时，前端通过 HTTP 获取全量状态；任务执行过程中，API 通过 WebSocket 推送增量事件。前端收到事件后静默刷新局部数据，避免页面反复进入加载态。

\subsection{Core BIZ 服务}
\label{subsec:implementation-biz-service}

Core BIZ 是系统中最复杂的服务，也是工程链路的状态中心。它负责项目创建、项目阶段推进、图像组管理、图像上传授权、图像上报、检测任务创建、人工复核、报告任务创建、对象存储访问、Redis 缓存、RocketMQ 事件生产、CronJob 定时任务和后台 Worker 调度器等。所有会改变业务状态的操作都集中在 BIZ 服务中执行。

BIZ 服务的核心设计是“服务层 + Repository 层 + Worker 层”。服务层实现 gRPC 接口和业务规则，Repository 层封装 MySQL、Redis、MinIO、RocketMQ 和 SMTP 操作，Worker 层异步推进检测和报告任务。Activity 服务回调 BIZ 后，BIZ 在数据库事务中写入结果、判断聚合状态、创建下一阶段任务并发布项目事件。
...
Classification 服务封装分类 Agent。BIZ 启动分类任务时传入任务 UUID、图像 UUID 和原图预签名下载地址。如\cref{fig:implementation-classification-service-sequence} 所示，Classification 服务下载并压缩图像后，通过 Agent 平台文件上传接口上传附件，再通过流式对话接口获取模型输出。服务端将 Agent 输出归一化为 JSON 对象，解析 \texttt{task\_codes} 任务代码，转换为共享枚举后回调 BIZ。
...
\begin{figure}[!htb]
  ...
  \resizebox{\linewidth}{!}{%
    \begin{tikzpicture}[
      ...
      \node[infra] (minio) at (5.10, 0) {MinIO\\图像原图};
      ...
      \draw[msg] (0,-0.90) -- node[label, above] {StartClassification(TaskUuid, ImageUuid, Url)} (2.47,-0.90);
      \draw[msg] (2.63,-1.55) -- (3.48,-1.55) -- node[label, right, pos=0.5] {加载 Prompt / 校验 BotId 与 Token / 创建请求上下文} (3.48,-2.15) -- (2.63,-2.15);
      ...
      \draw[ret] (7.57,-6.05) -- node[label, below] {FileId} (2.63,-6.05);
      \draw[msg] (2.63,-6.75) -- node[label, above] {提示词 + FileId 流式调用分类 Agent} (10.12,-6.75);
      \draw[ret] (10.12,-7.45) -- node[label, below] {模型输出：JSON 字符串} (2.63,-7.45);
      \draw[msg] (2.63,-8.15) -- (3.48,-8.15) -- node[label, right, pos=0.5] {CompactAgentJSONObjectString / 解析 TaskCodes} (3.48,-8.75) -- (2.63,-8.75);
      \draw[msg] (2.63,-9.45) -- (3.48,-9.45) -- node[label, right, pos=0.5] {非法代码拦截 / 去重 / 固定顺序排序 / 空集合允许} (3.48,-10.05) -- (2.63,-10.05);
      \draw[ret] (2.47,-10.75) -- node[label, below] {ReportClassificationResult(TaskCodes / Error)} (0,-10.75);
    \end{tikzpicture}
  }
  \caption{Classification 服务核心时序图}\label{fig:implementation-classification-service-sequence}
\end{figure}

分类 Agent 只决定任务代码，不创建子任务。输出异常时，Classification 服务回调失败状态；输出成功时，BIZ 仍要进行枚举转换、任务去重和子任务创建。这样既利用 Agent 对幕墙材质的语义判断能力，又避免 Agent 越过业务状态机。
...
\begin{figure}[!htb]
  ...
  \resizebox{\linewidth}{!}{%
    \begin{tikzpicture}[
      ...
      \draw[msg] (0,-0.90) -- node[label, above] {StartReasoning(TaskCode, ImageUuid)} (2.07,-0.90);
      \draw[msg] (2.23,-1.60) -- node[label, above] {按 TaskCode 查询执行器与产物规则} (4.22,-1.60);
...
Reasoning 服务是 Golang 工程层与 Python 模型层之间的桥梁。服务启动时注册五类原子检测能力，并根据任务代码检查模型目录中对应权重文件是否存在、是否可读取、是否为 Git LFS 指针文件。如\cref{fig:implementation-reasoning-service-sequence} 所示，任务执行时，Reasoning 服务从 BIZ 获取最新原图下载地址，下载到隔离运行目录，再调用常驻 Python Worker 执行对应检测模块。模型执行结束后，Golang 层读取压缩后的 \texttt{report.json}，再从 BIZ 获取产物上传授权，将目录中的产物并发上传到 MinIO，并带着结果 JSON 和产物清单回调 BIZ。

Reasoning 服务在真正开始执行检测时，才向 BIZ 申请原图下载地址和产物上传授权。这样做是因为检测任务可能在队列中等待一段时间，如果启动任务时就把预签名 URL 传入 Reasoning，等模型实际执行时地址可能已经过期，进而导致原图下载或产物上传失败。对于除玻璃爆裂以外的检测任务，Reasoning 还会检查产物是否为空、是否全部上传成功；如果产物不完整，则回调失败状态，而不是把缺失产物的结果当作成功任务。

Python 侧模型采用懒加载方式运行。Golang 服务保持常驻 Worker，并把具体检测任务以 JSON 请求形式发送给 Python 执行；Python Worker 从标准输入读取任务后，根据 \texttt{task\_code} 定位到 \texttt{python/detectors/\{task\_code\}/main.py}，再通过动态导入加载对应 detector 模块。每个 detector 都对外提供统一的 \texttt{detect(image\_path)} 入口，内部可以自由使用 PyTorch、OpenCV 或其他图像处理逻辑，但输出仍约定写入当前任务运行目录下的 \texttt{report.json} 和相关产物图像。首次调用某类 detector 时，Python 才加载对应模型权重，并将模块缓存在内存中；后续同类任务会复用已加载 detector。这样既可以保留 Python 在图像处理和深度学习模型调用方面的优势，也避免每次检测都重新加载模型权重带来的额外延迟。
...
\begin{figure}[!htb]
  ...
  \resizebox{\linewidth}{!}{%
    \begin{tikzpicture}[
      ...
      \draw[ret] (7.12,-6.10) -- node[label, below] {FileId / 输入上下文} (2.48,-6.10);
      ...
      \draw[msg] (2.48,-9.50) -- (3.35,-9.50) -- node[label, right, pos=0.5] {项目报告校验 GroupCount、ImageCount 与源数据一致} (3.35,-10.10) -- (2.48,-10.10);
...
Core Web 使用 React 和 TypeScript 实现，基于 Vite 构建。前端页面按项目阶段组织，包括项目基础信息、图像资产构建、Agent 智能检测、人工复核确认和评估报告生成。前端类型尽量复用 Common 中生成的 TypeScript 结构和枚举，避免手写常量与后端不一致。HTTP 请求统一经过 Axios 封装，处理令牌刷新、错误提示和 API 地址配置。
...
\begin{figure}[!htb]
  ...
  \resizebox{\linewidth}{!}{%
    \begin{tikzpicture}[
      ...
      \node[service] (go) at (6.90, 0) {Golang 服务\\API / BIZ / Activity};
...
在 MySQL 设计中，本文按照第三范式拆分业务表，将项目、图像组、图像、检测主任务、检测子任务、复核记录和报告记录分别建模。每张表只保存与自身实体直接相关的字段，跨实体关系通过主键、外键或业务 UUID 关联，而不是在一张宽表中重复保存项目名称、图像路径、任务状态和检测结果。这样可以减少数据冗余，也能避免项目基础信息修改后多处字段不一致的问题。对于区域指标、产物清单等结构不完全固定的数据，系统仍保留 JSON 字段，但这些字段只承担局部扩展作用，不替代核心业务实体建模。

\subsection{数据库设计与实现}
\label{subsec:implementation-database-design-and-implementation}

如\cref{tab:implementation-core-entities} 所示，数据库设计围绕“项目级资产”和“图像级任务”展开。项目表保存项目名称、建筑名称、位置、建成年份、建筑描述、已知问题、评估目标、项目状态和项目进度；图像组表保存项目内图像分组名称、用户归属和排序信息，用于表达立面、区域或用户自定义图像集合；图像表保存单张图像的 UUID、所属图像组、文件名、尺寸、大小、上传状态和元数据，是后续检测主任务、检测子任务和人工复核的图像级数据入口；检测主任务表保存每张图像的检测整体状态、五类子任务是否执行、子任务 ID、图像总结任务 ID 和人工复核字段；五张子任务表分别保存不同原子检测能力的专属指标；图像总结任务表保存图像级总结；项目报告表保存项目级报告 JSON 和状态。

\begin{table}[!htb]
  \caption{核心数据库实体与作用}\label{tab:implementation-core-entities}
  \centering
  \begin{tabular}{@{}p{0.32\linewidth}p{0.62\linewidth}@{}} \toprule
    \textbf{核心数据实体} & \textbf{作用} \\ \midrule
...
Redis 在本系统中主要保存一些有效期较短、但访问频率较高的数据。安全相关状态占了一部分，例如邮箱验证码、登录失败计数、账号短时锁定、Access Token 黑名单和 Refresh Token 刷新互斥锁。邮箱验证码会按照登录、注册、重置密码等场景分别保存，验证成功后立即消费，避免同一个验证码被重复使用；登录失败计数用于限制密码或验证码的连续尝试；Refresh Token 刷新互斥锁则用于处理并发刷新问题，防止同一个 Refresh Token 在短时间内被多个请求同时换新。

另一部分缓存与对象访问有关。前端查看图像资产、检测结果或项目缩略图时，经常会反复请求同一张原图、缩略图或检测产物。如果每次都重新生成预签名下载 URL，会增加 BIZ 与对象存储之间的重复操作。因此，系统会把部分预签名 URL 暂存在 Redis 中，并为其设置较短 TTL。Socket Ticket 和上传中的图像状态也放在 Redis 中保存，它们只用于连接建立或上传保护，不作为长期业务事实。真正需要长期保存和查询的数据仍以 MySQL 为准；删除图像、重试检测或清理对象时，BIZ 会同步删除相关 URL 缓存，避免前端继续拿到旧对象地址。
...
Reasoning 链路中比较容易出问题的是产物与结果不同步。模型可能已经给出了指标，但产物图像上传失败；也可能某个产物上传成功，却缺少可校验的 SHA256。为避免页面出现“任务成功但看不到产物”的情况，Reasoning 服务在回调时会把产物名称、对象 Key、SHA256 和上传结果一并返回给 BIZ。BIZ 不会只根据模型是否运行结束来判断子任务成功，而是继续检查产物清单。

对于需要产物的检测任务，只有全部必要产物都满足 \texttt{uploaded=true}，并且 SHA256 非空时，BIZ 才会写入成功结果并把子任务置为成功。只要有一个必要产物缺失、上传失败或校验信息不完整，该子任务就按失败处理，成功结果也不会落库。后续用户可以通过重试重新执行检测并上传产物。对象存储和 Redis 缓存也在这里配合使用：图像和检测产物可能被前端反复查看，系统会缓存部分预签名下载 URL；当图像被删除或脏对象被清理时，BIZ 同时清理对应缓存，避免已经失效的对象地址继续被使用。

\section{系统状态机设计与实现}
\label{sec:implementation-system-state-machine-design-and-implementation}

本系统的流程不能只靠前端页面顺序来约束。用户看见的只是“上传图像”“开始检测”“确认复核”“生成报告”等按钮，但按钮背后还有分类路由、模型推理、产物上传、图像总结和报告生成等后台步骤。某一步提前执行、重复执行或被跳过，都会影响后续数据是否可信。因此，本文把这些过程放到后端状态机中管理。如前文\cref{fig:requirements-stage-state-machine} 所示，系统中实际使用了项目工作流主状态机、图像资产构建状态机、检测主任务状态机、检测子任务状态机和评估报告生成状态机。这样做的目的不是把流程做复杂，而是让每个阶段都有可检查的进入条件和退出结果。

项目主流程从项目初始化开始，依次推进到基础信息完成、图像资产完成、检测完成、复核完成和项目完成。每次阶段变化前，Core BIZ 都会重新读取数据库中的实际状态，而不是只接收前端提交的阶段值。比如，基础信息阶段要确认项目资料已经保存；图像资产阶段至少要有一张上传成功的图像；检测阶段要等待相关检测主任务全部结束；复核阶段要确认用户已经完成复核；报告阶段只有在报告生成成功后，项目才会被标记为完成。前端按钮主要用于提示用户当前可以做什么，最终是否允许推进，仍然由后端根据数据条件判断。

检测主任务状态机对应的是单张图像从提交到总结完成的过程。主任务状态包括 \texttt{pending}、\texttt{classifying}、\texttt{detecting}、\texttt{summarizing}、\texttt{succeeded} 和 \texttt{failed}。任务刚创建时处于 \texttt{pending}，随后进入 \texttt{classifying} 调用分类 Agent。分类 Agent 返回任务代码后，BIZ 根据这些代码创建检测子任务。如果一张图像同时需要做多个检测，主任务会停留在 \texttt{detecting}，直到必要子任务全部结束。子任务全部成功后，系统再进入 \texttt{summarizing} 创建图像级总结；总结成功后，主任务才进入 \texttt{succeeded}。分类、检测或总结任一必要环节失败时，主任务进入 \texttt{failed}，并保存失败原因，供前端展示和后续重试使用。

检测子任务的状态相对简单，只包括 \texttt{pending}、\texttt{succeeded} 和 \texttt{failed}。需要注意的是，Reasoning 服务回调成功并不代表子任务一定成功，BIZ 还要继续检查产物完整性、上传状态和指标字段是否合法。评估报告生成也采用 \texttt{pending}、\texttt{succeeded} 和 \texttt{failed} 三种状态。报告生成中，前端可以刷新查看当前进度；报告失败后，页面显示重试入口；报告成功后，则不再提供刷新或重试操作。借助这些状态记录，系统可以把多服务异步执行过程落到数据库中，后续查询、失败定位和重试处理都会更清楚。

\section{安全性设计与实现}
\label{sec:implementation-security-design-and-implementation}

本系统的安全性设计主要围绕几个真实使用中容易出问题的位置展开：用户能不能被正确识别，项目数据会不会被越权访问，图像和检测产物的对象地址会不会长期暴露，长连接是否会绕过鉴权，以及 Agent 或模型服务返回的内容能不能直接影响数据库状态。由于系统涉及前端、Core API、Core BIZ、多个 Activity 服务、对象存储和消息队列，安全校验不能只放在某一个入口上，而是需要分散到请求接入、业务判断、对象授权和结果回调等环节中。

用户登录采用 Access Token 与 Refresh Token 配合的方式。Access Token 是有效期较短的 JWT，里面保存用户 ID、邮箱、用户名、签发时间、过期时间、签发方和 JTI\footnote{JTI 即 JWT ID，是 JWT 中用于标识令牌唯一编号的字段。系统可以利用该字段进行令牌追踪、吊销和黑名单校验。} 等信息，并使用 HMAC 签名算法\footnote{HMAC 是一种基于密钥的消息认证码算法。服务端使用密钥对消息生成签名，并在校验时重新计算签名，从而判断令牌内容是否被篡改。} 做完整性校验。Refresh Token 则用于换取新的访问令牌，它本身是随机生成的长期令牌，明文只返回给客户端，数据库只保存其哈希值和 Token ID。这样即使数据库中保存的记录被读取，也不能直接还原出客户端持有的 Refresh Token。

API 鉴权中间件主要校验 Access Token。当前端发现访问令牌即将过期，或者业务请求返回 401 时，再通过 \texttt{/auth/refresh} 使用 HttpOnly Cookie 中的 Refresh Token 换取新的令牌。这里没有把 Refresh Token 暴露给前端 JavaScript 直接读取，一方面可以减少脚本窃取令牌的风险，另一方面也便于服务端统一控制刷新、吊销和过期处理。
...
\begin{figure}[!htb]
  ...
  \caption{用户登录鉴权与令牌刷新安全流程}\label{fig:implementation-auth-token-security-flow}
\end{figure}

如\cref{fig:implementation-auth-token-security-flow} 所示，令牌刷新并不是在原令牌上直接延长时间，而是采用轮换方式。BIZ 会先在 Redis 中为同一个 Refresh Token 设置短时刷新锁，避免浏览器并发请求同时触发刷新。刷新成功后，系统签发新的 Access Token 和 Refresh Token，同时吊销旧的 Refresh Token。用户主动登出时，尚未过期的 Access Token 会按照 JTI 写入 Redis 黑名单，对应的 Refresh Token 也会被吊销。这样处理后，令牌即使在有效期内，也可以被服务端提前失效。

项目权限没有放给前端自行判断。所有项目相关接口都会携带用户身份和项目 ID，Core BIZ 根据数据库中的项目归属关系判断该用户是否可以读取、修改或推进项目阶段。前端隐藏按钮只能改善交互体验，不能作为权限依据。这样做的原因很直接：用户可以刷新页面、修改请求参数，甚至绕过页面直接调用接口，所以最终判断必须回到后端。

对象存储访问也采用短时授权。图像原图、缩略图、检测产物和报告源文件都保存在 MinIO 中，前端并不持有 MinIO 的长期账号或密钥。上传图像时，BIZ 生成预签名 UPLOAD URL；查看图像或产物时，系统再生成预签名下载 URL；Reasoning 上传检测产物和 Summary 读取报告源数据时，也只拿到限定对象或限定前缀的临时授权。即使某个 URL 被复制出去，它的作用范围和可用时间也比较有限。

密码和验证码的处理尽量避免保存可直接使用的敏感信息。注册和重置密码时，BIZ 会先检查密码长度和复杂度，再使用 Bcrypt 生成密码哈希写入数据库；登录时只比对哈希值，不保存明文密码。邮箱验证码按登录、注册和重置密码等场景分别生成，并保存在 Redis 中。验证码通过后会被消费，不能继续用于其他场景或再次提交，这样可以减少验证码复用带来的风险。

WebSocket 连接没有直接把 Access Token 拼接到长连接地址中。前端需要先调用受鉴权保护的 \texttt{/socket/ticket} 接口，BIZ 校验用户身份和项目权限后，生成一个短时 Ticket。建立连接时，前端同时提交项目 ID、Scope 和 ticket，API 再向 BIZ 校验票据是否有效。Scope 用于区分图像资产、检测任务和报告状态等事件范围，避免用户打开某个页面时接收不属于当前阶段的全量消息。

Agent 和模型服务的输出同样不能直接进入数据库。分类 Agent 返回的内容需要被解析成系统允许的任务代码，非法代码会被过滤或拒绝；图像总结和项目报告需要压缩为合法 JSON 对象后才能回调写入；项目报告还会检查图像组数量、图像数量等字段是否与源数据一致。Reasoning 服务启动时会检查模型文件，避免部署目录中只有 Git LFS 指针文件而没有真实权重。上述处理不能完全消除外部服务的不确定性，但可以避免未经校验的文本、任务代码或产物状态直接改变业务事实。

因此，本节的安全实现可以概括为两个思路：入口处先确认“是谁在访问、能不能访问”，业务执行时再确认“访问的对象、返回的结果和推进的状态是否可信”。用户身份、项目权限、对象授权、WebSocket Ticket 和 Agent 输出校验分别处理不同风险点，最后都回到 Core BIZ 做关键判断。这样既不需要前端保存过多敏感信息，也能减少越权访问、凭证泄露和非受控输出对系统状态造成的影响。

\section{稳定性设计与实现}
\label{sec:implementation-stability-design-and-implementation}

稳定性设计覆盖日志、异常处理、容量控制、超时控制、幂等重试、产物校验和清理机制等方面。系统的稳定性基本原则是：将长链路任务中的失败、超时和中间产物异常视为常态，通过可观测日志、受控并发、明确超时、状态机推进、事务一致性、失败重试和后台清理机制，使系统在局部异常发生时仍然能够定位问题、保留业务事实并恢复到可继续执行的状态。

各服务按 HTTP、RPC、MQ、WS、CRON、CLS、RSN、SMR 等作用域范围输出结构化日志，并保留请求 ID、任务 UUID、图像 UUID、状态、耗时和错误信息。Docker Compose 中为五个后端服务配置日志采集 Sidecar，将容器日志按 PSM 写入 \texttt{/root/logs/\{psm\}/log}，并按文件大小和最大文件数量进行轮转。实际排查问题时，按请求 ID、任务 UUID 或图像 UUID 追踪具体链路。

在容量控制方面，Classification、Reasoning、Summary 和 Detection Worker 都有最大并发配置。Agent 调用和模型推理属于高耗时任务，如果不限制并发，容易压垮外部 Agent 平台、Python 模型进程、对象存储和数据库。通过并发上限和任务队列，系统在吞吐量与稳定性之间取得平衡。

长任务中最常见的问题是等待时间过长或临时授权过期。因此，Agent 请求、Reasoning 总任务、产物下载、产物上传、图像上传待确认、Socket Ticket 和预签名 URL 都设置了各自的 TTL 或 Timeout。Reasoning 服务没有在任务创建时就保存原图下载地址和上传地址，而是在真正开始执行时再向 BIZ 获取最新授权，这样可以减少队列等待造成的 URL 过期问题。对于上传中但迟迟没有上报完成的图像，上传超时 CronJob 会把这类记录标记为失败，并把状态推送给前端，避免页面一直停留在“上传中”。

状态更新集中放在 BIZ 中处理。Classification、Reasoning 和 Summary 等 Activity 服务完成任务后，只通过回调把结果交给 BIZ，不直接修改数据库。BIZ 收到回调后，会先检查任务是否还存在、当前状态是否允许被替换、产物清单是否完整，再在事务中写入结果、推进状态并发布事件。Reasoning 产物的处理更严格一些，只有全部必要产物都上传成功，子任务才会被置为成功；如果产物缺失或对象不存在，即使模型已经返回指标，也不会写入成功结果。这样可以减少前端结果页出现缺图、空链接或对象不存在的情况。

检测和报告都保留失败状态，并允许用户在失败后重新触发。检测任务失败时，前端可以重新启动失败任务；报告生成失败时，页面显示重试入口，由 BIZ 重新创建报告任务并调用 Summary 服务。对象存储侧还需要单独清理长期遗留文件。MinIO 脏对象清理 CronJob 会扫描失败图像、失败子任务、失败报告以及没有数据库引用的对象文件夹，并在保护窗口之后再执行删除，避免正在上传或正在检测的对象被误删。通过这种方式，系统可以在失败后保留必要状态，同时控制对象存储持续膨胀。

系统也提供了一些基础的可观测信息。工作台会统计项目数量、图像数量、图像组数量、中间产物数量、检测执行次数、报告生成次数和对象存储容量。它能够让用户和维护者快速看到当前账号下的数据规模和存储占用。

因此，系统稳定性并非只由单个服务的异常捕获实现，而是由状态机、事务、队列、超时、重试、日志、清理任务和容量控制共同支撑，使长链路 Agent 任务在失败、超时或产物异常时仍能被定位、回滚、重试和恢复。

\section{部署设计与实现}
\label{sec:implementation-deployment-implementation}

本项目部署时使用 Docker Compose 统一拉起各个服务。Compose 文件中除了 Core Web、Core API 和 Core BIZ 外，还包含 RocketMQ NameServer、Broker、Topic 初始化服务、三个 Activity 服务以及日志采集 Sidecar。各容器被放在同一个 Docker 网络中，服务之间直接通过容器服务名访问，不需要在代码中写固定 IP。对外访问主要经过两个入口：Core API 暴露 HTTP 接口，Core Web 由 Nginx 托管前端静态资源并映射到 80 端口。Reasoning 服务比较特殊，它需要读取服务器上的模型权重，因此通过只读挂载方式加载模型目录，避免模型文件被容器运行过程意外修改。
...
\begin{figure}[!htb]
  ...
  \resizebox{\linewidth}{!}{%
    \begin{tikzpicture}[
      font=\scriptsize,
      >=stealth,
      host/.style={draw=gray!70!black, fill=gray!5, rounded corners=4pt, line width=0.65pt},
      network/.style={draw=purple!55!black, fill=purple!4, rounded corners=4pt, line width=0.65pt},
      access/.style={draw=blue!65!black, fill=blue!7, rounded corners=3pt, align=center, minimum width=2.20cm, minimum height=0.72cm, line width=0.55pt},
      core/.style={draw=green!55!black, fill=green!8, rounded corners=3pt, align=center, minimum width=2.20cm, minimum height=0.72cm, line width=0.55pt},
      activity/.style={draw=orange!75!black, fill=orange!10, rounded corners=3pt, align=center, minimum width=2.20cm, minimum height=0.72cm, line width=0.55pt},
      mq/.style={draw=red!65!black, fill=red!7, rounded corners=3pt, align=center, minimum width=2.20cm, minimum height=0.72cm, line width=0.55pt},
      volume/.style={draw=gray!75!black, fill=gray!10, rounded corners=3pt, align=center, minimum width=2.70cm, minimum height=0.72cm, line width=0.55pt},
      label/.style={font=\bfseries\scriptsize, anchor=west},
      flow/.style={->, line width=0.58pt},
      mount/.style={->, dashed, line width=0.55pt},
      note/.style={font=\tiny, align=center, inner sep=1pt}
    ]
      \path[use as bounding box] (-9.75,-4.25) rectangle (9.05,4.75);

      \draw[host] (-7.20,-3.55) rectangle (8.95,4.55);
      \node[label] at (-6.88,4.18) {服务器部署环境};

      \draw[network] (-6.75,-1.55) rectangle (8.55,3.85);
      \node[label] at (-6.43,3.48) {Docker Compose 服务网络：icw.docker.network};

      \node[access] (user) at (-8.55,2.55) {浏览器用户\\HTTP / WS};
      \node[access] (web) at (-5.15,2.55) {Core Web\\Nginx: 80};
      \node[access] (api) at (-2.25,2.55) {Core API\\HTTP / WS: 8000};

      \node[core] (biz) at (-2.25,1.05) {Core BIZ\\gRPC: 8001};
      \node[activity] (classification) at (-5.15,-0.70) {Classification\\gRPC: 8002};
      \node[activity] (reasoning) at (-2.25,-0.70) {Reasoning\\gRPC: 8003};
      \node[activity] (summary) at (0.65,-0.70) {Summary\\gRPC: 8004};

      \node[mq] (init) at (3.45,2.55) {Topic Init\\ICW\_PROJECT\_EVENT};
      \node[mq] (broker) at (3.45,1.05) {RocketMQ\\Broker: 10911};
      \node[mq] (namesrv) at (7.10,1.05) {RocketMQ\\NameServer: 9876};
      \node[volume] (logsidecar) at (5.625,-0.70) {Log Sidecar\\五个业务日志容器};

      \node[label] at (-6.88,-2.05) {宿主机挂载与外部依赖};
      \node[volume] (models) at (-5.40,-2.85) {\texttt{/root/models}\\只读模型目录};
      \node[volume] (storage) at (-2.25,-2.85) {MySQL / Redis / MinIO\\状态、缓存与对象};
      \node[volume] (config) at (0.90,-2.85) {环境变量\\地址 TTL 并发};
      \node[volume] (logs) at (4.05,-2.85) {\texttt{/root/logs}\\分 PSM 日志};
      \node[volume] (socket) at (7.20,-2.85) {Docker Socket\\只读日志源};

      \draw[flow] (user.east) -- (web.west);
      \draw[flow] (web.east) -- node[note, above] {HTTP} (api.west);
      \draw[flow] (api.south) -- node[note, right] {gRPC} (biz.north);

      \draw[flow] (biz.south west) -- node[note, above left] {gRPC} (classification.north);
      \draw[flow] (biz.south) -- node[note, right] {gRPC} (reasoning.north);
      \draw[flow] (biz.south east) -- node[note, above right] {gRPC} (summary.north);

      \draw[flow] (biz.east) -- node[note, above] {发布项目事件} (broker.west);
      \draw[flow] (broker.north) -- node[note, right] {健康后初始化} (init.south);
      \draw[flow] (namesrv.west) -- node[note, above] {路由发现} (broker.east);
      \draw[flow] (broker.north west) -- node[note, above] {消费并广播} (api.east);

      \draw[mount] (models.north east) -- (reasoning.south);
      \draw[mount] (socket.north) -- (logsidecar.south east);
      \draw[mount] (logsidecar.south west) -- (logs.north);
      \draw[mount] (biz.south east) .. controls (-0.82,0.12) and (-0.82,-2.15) .. (storage.north east);
      \draw[mount] (config.north) -- node[note, right] {注入运行参数} (0.90,-1.55);
    \end{tikzpicture}
  }
  \caption{系统部署视图}\label{fig:implementation-deployment-view}
\end{figure}

如\cref{fig:implementation-deployment-view} 所示，部署视图可以分为三部分：对外入口由 Core Web 和 Core API 承担，内部服务通过 Docker bridge 网络按服务名互访，模型目录、日志目录、Docker Socket 和数据服务则作为宿主机或外部依赖接入容器运行环境。该部署方式既保持了服务间通信地址的稳定性，也把模型权重、日志文件和运行参数从镜像中拆分出来，便于后续更新和排查。

服务启动顺序主要依靠 \texttt{depends\_on} 和健康检查控制。RocketMQ Broker 先启动并通过健康检查后，Topic 初始化服务再创建项目事件需要使用的 Topic。Classification、Reasoning、Summary、Core BIZ 和 Core API 都会等待 Topic 初始化完成后再启动，避免服务刚启动就因为消息队列不可用而报错。Core BIZ 还会等待三个 Activity 服务启动，因为检测 Worker 后续需要调用这些服务。

运行参数没有硬编码在程序中，而是集中放到环境变量里。服务地址、MySQL、Redis、MinIO、RocketMQ、JWT、邮箱验证码 Secret、Socket Ticket、对象存储 TTL、Agent Token、Agent Bot ID、任务超时和并发数量等都可以通过环境变量调整。本地调试和服务器部署只需要更换配置值，不必修改代码重新编译，也便于后续在其他机器上复现实验环境。

Reasoning 模型文件通过 \texttt{/root/models:/app/models:ro} 方式挂载进 Docker 镜像。这样可以避免模型权重导致镜像体积过大，也便于更新模型文件。服务启动时会检查模型文件是否存在且不是 Git LFS 指针，提前暴露部署问题。
...
本章主要说明\cref{chap:design}中的技术方案如何落到具体工程实现中。实现时，系统被拆分为 Core Web、Core API、Core BIZ、Activity Classification、Activity Reasoning、Activity Summary 和 ICW Common 七个服务与模块。这样的拆分不是单纯为了增加服务数量，而是为了让前端交互、业务状态、Agent 调用、模型推理、报告生成和共享协议各自有明确位置。Core BIZ 负责控制任务状态和数据库事务，Activity 服务只返回结构化结果，Core API 负责 HTTP 接入、鉴权和实时推送，Core Web 负责阶段化展示、检测结果查看和人工复核。

从实现过程看，难点主要不在于某一个单独接口，而在于系统的长链路协同。一次检测任务会经过 HTTP 请求、gRPC 调用、Worker 入队、分类 Agent、子任务创建、Python 模型执行、MinIO 产物上传、回调落库、RocketMQ 事件、WebSocket 推送和前端状态合并。项目报告任务还要在此基础上组装项目级源数据，保存多源检测结构化结果，调用附件式 Agent，校验报告 JSON，并把报告内容展示给用户。为支撑这些流程，系统在协议、状态机、对象存储、CronJob 定时任务、日志、并发控制和 Docker Compose 部署上都做了相应实现，使 Agent 能力转化为可运行、可追踪、可恢复的软件系统。
