...
总体而言，本系统的安全性设计并不依赖单一鉴权点，而是将用户身份、项目权限、对象访问、长连接票据和 Agent 输出约束组合起来，形成覆盖“用户、数据、文件、服务和智能输出”的多层安全边界。该设计将安全校验前移到访问入口，同时将关键业务决策收束到 Core BIZ 中执行，避免前端状态或外部 Agent 输出直接影响数据库事实。通过短期授权、范围隔离和结构化校验，系统在保证交互便利性的同时，降低了越权访问、凭证泄露和非受控输出带来的工程风险。
...
本章从工程角度说明了\cref{chap:design}技术方案的落地方式。系统通过 Core Web、Core API、Core BIZ、Activity Classification、Activity Reasoning、Activity Summary 和 ICW Common 七个服务或模块，将前端交互、业务状态、Agent 调用、模型推理、报告生成和共享协议拆分到清晰边界中。BIZ 服务集中控制任务状态和数据库事务，Activity 服务只返回结构化结果，API 服务负责接入与实时推送，Web 服务负责阶段化展示和用户复核。

工程实现的主要工作量体现在长链路协同上：一次检测任务涉及 HTTP 请求、gRPC 调用、Worker 入队、分类 Agent、子任务创建、Python 模型执行、MinIO 产物上传、回调落库、RocketMQ 事件、WebSocket 推送和前端状态合并；一次项目报告任务还涉及项目级源数据组装、多源检测结构化数据存储、附件式 Agent 调用、报告 JSON 校验和报告展示。通过协议治理、状态机、对象存储、CronJob、日志、并发控制和 Docker 部署，系统将 Agent 能力转化为可运行、可追踪、可恢复的软件系统。
