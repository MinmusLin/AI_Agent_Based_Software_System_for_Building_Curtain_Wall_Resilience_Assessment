...
Agent 引入工程系统后，问题并不只在于模型能不能完成任务，还在于它的输出能否被稳定接入业务流程。大语言模型本身具有概率性，实际使用时可能出现格式不稳定、内容越界、幻觉或工具选择错误等情况。已有研究对幻觉问题进行了较多讨论，其风险在于模型生成的内容表面上较为合理，但缺少事实依据，放入真实系统后会影响信息可靠性\cite{huang2023hallucination}。在工具调用场景中，模型还需要判断调用时机、工具类型和参数组织方式。Toolformer、ToolLLM 等研究表明，大语言模型可以学习或增强工具使用能力；但从工程角度看，这也意味着工具使用能力需要依赖工具集合、调用路径和评估机制进行约束\cite{schick2023toolformer,qin2023toolllm}。如果只是普通问答系统，上述问题更多体现为回答质量波动；如果 Agent 的输出会进一步影响数据库状态或任务调度，就可能带来错误流程。因此，结构化输出、工具白名单、状态隔离和人工复核，是 Agent 工程化中必须处理的基础问题。
...
如\cref{fig:foundation-agent-engineering-risk-control-mechanism} 所示，针对上述风险，现有研究和工程实践通常会从动作、输出和流程三个层面进行约束：

\begin{enumerate}
  \item 一是限制动作空间，让 Agent 只能在预定义工具、API 或任务代码中选择。ReAct 通过推理与行动交替形成较清晰的行动轨迹，AutoGen 则通过多 Agent 对话和人类参与机制组织复杂任务流程\cite{yao2023react,wu2023autogen}；
  \item 二是约束输出格式，使模型结果能够被程序稳定解析。PICARD 通过增量解析约束自回归解码，Outlines 相关研究则将文本生成表示为有限状态机转移，并借助正则表达式或上下文无关文法限制生成结构\cite{scholak2021picard,willard2023efficient}；
  \item 三是把 Agent 放入受控工作流，由工程系统负责状态推进、失败重试、日志记录和人工复核。相关综述也指出，LLM Agent 的规划、工具使用和反馈机制需要与评估和控制机制共同设计，才能进入可靠应用\cite{wang2023llmagents}。
\end{enumerate}
