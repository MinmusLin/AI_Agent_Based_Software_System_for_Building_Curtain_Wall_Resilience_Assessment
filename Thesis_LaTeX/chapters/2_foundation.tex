...
深度学习方法则通过数据驱动方式学习图像特征。卷积神经网络能够自动提取从边缘纹理到高层语义的多层特征，ResNet 等残差网络通过残差连接缓解深层网络训练困难，常用于图像分类任务\cite{article1,conf1}。目标检测模型能够同时输出目标类别和位置，基于候选区域的两阶段方法和以 YOLO、SSD 为代表的一阶段方法构成了深度学习目标检测的重要技术路线，国内已有综述对 R-CNN、Faster R-CNN、YOLO 和 SSD 等代表性算法进行了系统梳理\cite{fan2020objectdetection}，其中 YOLO 系列以端到端、实时检测为代表，适合缺陷候选区域快速定位\cite{redmon2016yolo}。语义分割和实例分割模型能够输出像素级区域，U-Net 等编码器-解码器结构为像素级分割提供了经典范式，相关中文综述已对深度学习语义分割的区域分类、像素分类、全监督和弱监督方法进行系统总结\cite{tian2019semanticsegmentation}，DeepLabV3+ 通过空洞卷积和编码器-解码器结构提升语义分割效果\cite{chen2018deeplabv3plus}，视觉 Transformer 通过自注意力机制增强长距离依赖建模能力，为视觉识别、目标检测和视觉分割等任务提供了新的表征基础\cite{zhou2023visiontransformer}，SegFormer 进一步将 Transformer 表征用于高效语义分割\cite{xie2021segformer}，Mask R-CNN 等实例分割方法和 Segment Anything 则分别从实例掩码预测和通用分割能力等方向扩展了区域分割能力\cite{su2022instancesegmentation,kirillov2023sam}。在建筑结构和幕墙场景中，裂缝检测、玻璃检测和表面缺陷识别等任务均已有较多研究。Ye 等人利用深度学习方法进行结构裂缝检测，为石材或混凝土裂缝识别提供了参考\cite{ye2019crack}；Mei 等人研究真实场景中的玻璃检测问题，对玻璃幕墙区域分析提供了参考\cite{mei2020glass}。
