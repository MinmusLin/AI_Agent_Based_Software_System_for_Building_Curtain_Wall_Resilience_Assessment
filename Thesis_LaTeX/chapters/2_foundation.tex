...
玻璃平整度检测的目标是识别玻璃幕墙是否存在局部变形、反射异常或不平整现象。玻璃材质具有透明、反光和低纹理等特点，真实图像中的不平整往往并不表现为明确边界，而是表现为边缘畸变、直线偏折、梯度分布异常和频域能量变化。因此，玻璃平整度检测脚本采用“玻璃区域识别 + 多特征规则判别”的实现路径。该流程先通过玻璃检测或分割模型获得玻璃区域，再对局部区域进行边缘、直线、梯度和频谱分析。
